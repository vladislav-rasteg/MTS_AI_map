import { SizedIconLayout } from "../SizedIconLayout";


interface IconProps{
    size?: number;
    fill?: string;
    className?: string;
}

export const LinkIcon = (props: IconProps) => {

    const {fill = "--icon-default-color", className} = props;

    const bodyStyles = window.getComputedStyle(document.body);
    const iconColor = bodyStyles.getPropertyValue(fill);

    return(
        <SizedIconLayout iconProps={props}>
            <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M20.44 3.46C20.3213 3.18364 20.0508 3.00329 19.75 3H14.75C14.3358 3 14 3.33579 14 3.75C14 4.16421 14.3358 4.5 14.75 4.5H17.94L12.22 10.22C11.9275 10.5128 11.9275 10.9872 12.22 11.28C12.5128 11.5725 12.9872 11.5725 13.28 11.28L19 5.56V8.75C19 9.16421 19.3358 9.5 19.75 9.5C20.1642 9.5 20.5 9.16421 20.5 8.75V3.75C20.4995 3.65031 20.4791 3.55172 20.44 3.46Z" fill={iconColor} className={className}/>
                <path d="M17.75 11C17.338 11.0054 17.0054 11.338 17 11.75V15.75C16.9945 17.5426 15.5426 18.9945 13.75 19H7.75C5.95735 18.9945 4.50549 17.5426 4.5 15.75V9.75C4.50549 7.95735 5.95735 6.50549 7.75 6.5H11.75C12.1642 6.5 12.5 6.16421 12.5 5.75C12.5 5.33579 12.1642 5 11.75 5H7.75C5.12665 5 3 7.12665 3 9.75V15.75C3 18.3734 5.12665 20.5 7.75 20.5H13.75C16.3734 20.5 18.5 18.3734 18.5 15.75V11.75C18.4946 11.338 18.162 11.0054 17.75 11Z" fill={iconColor} className={className}/>
            </svg>
        </SizedIconLayout>
    )
}